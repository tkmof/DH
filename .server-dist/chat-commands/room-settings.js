...
const MAX_CHATROOM_ID_LENGTH = 225;
 ...
 const commands = {
	...
	modchat(target, room, user) {
		...
		if (room.settings.modchat &&
				...
				Config.groupsranking.indexOf(room.settings.modchat ) > threshold
		) {
	...
	showmedia(target, room, user) {
		...
		if (this.meansNo(target)) {
			...
			room.settings.showEnabled = target ;
		}
		room.saveSettings();
	...
	hidenext(target, room, user) {
		const groupConfig = Config.groups[Users.PLAYER_SYMBOL];
		if (!_optionalChain([groupConfig, 'optionalAccess', _4 => _4.editprivacy])) return this.errorReply(`/hidenext - Access denied.`);
		if (this.meansNo(target)) {
	...
	unsubroom(target, room, user) {
		...
		if (_optionalChain([parent, 'optionalAccess', _5 => _5.subRooms])) {
 ...
 const roomSettings = [
	...
	(room, user) => {
		...
		const options = !permission ? [[room.settings.modchat || 'off', true]] :
			[
				'off',
				'autoconfirmed',
				'trusted',
