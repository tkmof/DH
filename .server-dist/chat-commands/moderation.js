...
const REQUIRE_REASONS = true;
 ...
 const commands = {
	...
	roompromote(target, room, user, connection, cmd) {
		...
		const nextSymbol = target === 'deauth' ? Users.Auth.defaultSymbol() : target ;
		...
		if (this.pmTarget && targetUser) {
			...
			_optionalChain([shouldPopup, 'optionalAccess', _2 => _2.popup, 'call', _3 => _3(`You were demoted to Room ${nextGroupName} by ${user.name} in ${room.roomid}.`)]);
		} else if (nextSymbol === '#') {
			this.addModAction(`${'' + name} was promoted to ${nextGroupName} by ${user.name}.`);
			this.modlog('ROOM OWNER', userid);
			_optionalChain([shouldPopup, 'optionalAccess', _4 => _4.popup, 'call', _5 => _5(`You were promoted to ${nextGroupName} by ${user.name} in ${room.roomid}.`)]);
		} else {
			this.addModAction(`${'' + name} was promoted to Room ${nextGroupName} by ${user.name}.`);
			this.modlog(`ROOM${nextGroupName.toUpperCase()}`, userid);
			_optionalChain([shouldPopup, 'optionalAccess', _6 => _6.popup, 'call', _7 => _7(`You were promoted to Room ${nextGroupName} by ${user.name} in ${room.roomid}.`)]);
		}
	...
	authority(target, room, user, connection) {
		if (target && target !== '+') {
			...
			const availableRoom = _optionalChain([targetRoom, 'optionalAccess', _8 => _8.checkModjoin, 'call', _9 => _9(user)]);
		...
		const buffer = _utils.Utils.sortBy(
			...
			return `${group}:\n` +
				_utils.Utils.sortBy(names).map(userid => {
					const isOnline = _optionalChain([Users, 'access', _10 => _10.get, 'call', _11 => _11(userid), 'optionalAccess', _12 => _12.statusType]) === 'online';
	...
	userauth(target, room, user, connection) {
		...
		const targetUsername = _optionalChain([targetUser, 'optionalAccess', _13 => _13.name]) || target;
	...
	unmute(target, room, user) {
		...
		const successfullyUnmuted = room.unmute(
			_optionalChain([targetUser, 'optionalAccess', _14 => _14.id]) || toID(this.targetUsername), `Your mute in '${room.title}' has been lifted.`
		);
	...
	ban(target, room, user, connection, cmd) {
		...
		if (targetUser.id in room.users || user.can('lock')) {
			targetUser.popup(
				`|modal||html|<p>${_utils.Utils.escapeHTML(user.name)} has banned you from the room ${room.roomid} ${(room.subRooms ? ` and its subrooms` : ``)}.</p>${(target ? `<p>Reason: ${_utils.Utils.escapeHTML(target)}</p>` : ``)}<p>To appeal the ban, PM the staff member that banned you${room.persist ? ` or a room owner. </p><p><button name="send" value="/roomauth ${room.roomid}">List Room Staff</button></p>` : `.</p>`}`
			);
		}
	...
	async lock(target, room, user, connection, cmd) {
		...
		_optionalChain([room, 'optionalAccess', _15 => _15.hideText, 'call', _16 => _16([userid, toID(this.inputUsername)])]);
		...
		// Automatically upload replays as evidence/reference to the punishment
		if (_optionalChain([room, 'optionalAccess', _17 => _17.battle])) this.parse('/savereplay forpunishment');
		return true;
	},
	lockhelp: [
	...
	unlock(target, room, user) {
		...
		if (_optionalChain([targetUser, 'optionalAccess', _18 => _18.namelocked])) {
			return this.errorReply(`User ${targetUser.name} is namelocked, not locked. Use /unnamelock to unnamelock them.`);
		}
		let reason = '';
		if (_optionalChain([targetUser, 'optionalAccess', _19 => _19.locked]) && targetUser.locked.charAt(0) === '#') {
	...
	async globalban(target, room, user, connection, cmd) {
		...
		if (targetUser.trusted) {
			if (cmd === 'forceglobalban') {
				const from = targetUser.distrust();
				Monitor.log(`[CrisisMonitor] ${name} was globally banned by ${user.name} and demoted from ${_optionalChain([from, 'optionalAccess', _20 => _20.join, 'call', _21 => _21(", ")])}.`);
				this.globalModlog("CRISISDEMOTE", targetUser, ` from ${_optionalChain([from, 'optionalAccess', _22 => _22.join, 'call', _23 => _23(", ")])}`);
			} else {
		...
		_optionalChain([room, 'optionalAccess', _24 => _24.hideText, 'call', _25 => _25([userid, toID(this.inputUsername)])]);
	...
	promote(target, room, user, connection, cmd) {
		...
		const currentGroup = _optionalChain([targetUser, 'optionalAccess', _26 => _26.group]) || Users.globalAuth.get(userid);
	...
	htmldeclare(target, room, user) {
		...
		for (const u in room.users) {
			_optionalChain([Users, 'access', _27 => _27.get, 'call', _28 => _28(u), 'optionalAccess', _29 => _29.sendTo, 'call', _30 => _30(
				room,
	...
	forcerename(target, room, user) {
		...
		if (_optionalChain([room, 'optionalAccess', _31 => _31.roomid]) !== 'staff') this.privateModAction(`(${targetUser.name} ${forceRenameMessage})`);
	...
	hidetext(target, room, user, connection, cmd) {
		...
		if (_optionalChain([targetUser, 'optionalAccess', _34 => _34.trusted]) && targetUser !== user && !cmd.includes('force')) {
	...
	blacklistname(target, room, user) {
		...
		const duplicates = targets.filter(userid => (
			_optionalChain([Punishments, 'access', _35 => _35.roomUserids, 'access', _36 => _36.nestedGet, 'call', _37 => _37(room.roomid, userid), 'optionalAccess', _38 => _38[0]]) === 'BLACKLIST'
		));
