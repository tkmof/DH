...
var _processmanager = require('../../.lib-dist/process-manager'); var ProcessManager = _processmanager;
 ...
 const commands = {
	...
	async hotpatch(target, room, user, connection, cmd) {
		...
		const requiresForceMessage = `The git work tree has not changed since the last time ${target} was hotpatched (${_optionalChain([version, 'optionalAccess', _ => _.slice, 'call', _2 => _2(0, 8)])}), use /forcehotpatch ${target} if you wish to hotpatch anyway.`;
	...
	kill(target, room, user) {
		...
		if (!_optionalChain([logRoom, 'optionalAccess', _3 => _3.log, 'access', _4 => _4.roomlogStream])) return process.exit();
		...
		const exec = (command) => {
			...
			return new Promise((resolve, reject) => {
				child_process.exec(command, {
					...
					if (error) log = `[c] ${error.code}\n${log}`;
					this.stafflog(log);
					resolve([_optionalChain([error, 'optionalAccess', _6 => _6.code]) || 0, stdout, stderr]);
				});
			});
		};
	...
	async eval(target, room, user, connection) {
		...
		_optionalChain([logRoom, 'optionalAccess', _7 => _7.roomlog, 'call', _8 => _8(`>> ${target}`)]);
		try {
			...
			_optionalChain([logRoom, 'optionalAccess', _10 => _10.roomlog, 'call', _11 => _11(`<< ${result}`)]);
		} catch (e) {
			...
			_optionalChain([logRoom, 'optionalAccess', _12 => _12.roomlog, 'call', _13 => _13(`<< ${message}`)]);
		}
	},
