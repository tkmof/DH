...
// User
 class User extends Chat.MessageContext {
	...
	constructor(connection) {
		...
		this.id = '';
		this.group = _usergroups.Auth.defaultSymbol();
		this.language = null;
	...
	can(permission, target = null, room = null) {
		...
		const auth = room ? room.auth : exports.Users.globalAuth;

		let group = auth.get(this);
		if (auth.has(this.id) && group === _usergroups.Auth.defaultSymbol()) {
			group = 'whitelist' ;
		}
		...
		if (roomIsTemporary && group === this.group) {
			const replaceGroup = _usergroups.Auth.getGroup(group).globalGroupInPersonalRoom;
			if (replaceGroup) group = replaceGroup;
		}

		return _usergroups.Auth.hasPermission(group, permission , targetGroup, target === this);
	}
	/**
	...
	updateGroup(registered) {
		if (!registered) {
			this.registered = false;
			this.group = exports.Users.Auth.defaultSymbol();
			this.isStaff = false;
			return;
		}
	...
	setGroup(group, forceTrusted = false) {
		...
		Rooms.global.checkAutojoin(this);
		if (this.registered) {
			if (forceTrusted || this.group !== exports.Users.Auth.defaultSymbol()) {
