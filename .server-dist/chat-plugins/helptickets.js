...
function notifyStaff() {
	...
	for (const key of keys) {
		...
		const creator = ticket.claimed ? _utils.Utils.html`${ticket.creator}` : _utils.Utils.html`<strong>${ticket.creator}</strong>`;
 ...
 const pages = {
	help: {
		request(query, user, connection) {
			...
			if (_optionalChain([ticket, 'optionalAccess', _ => _.open]) || ipTicket) {
		...
		tickets(query, user, connection) {
			...
			for (const key of banKeys) {
				...
				buf += _utils.Utils.html`<td>${ticket.name}</td>`;
				buf += _utils.Utils.html`<td>${ticket.by}</td>`;
 ...
 const commands = {
	report(target, room, user) {
		...
		if (this.broadcasting) {
			if (_optionalChain([room, 'optionalAccess', _2 => _2.battle])) return this.errorReply(`This command cannot be broadcast in battles.`);
	...
	appeal(target, room, user) {
		...
		const meta = this.pmTarget ? `-user-${this.pmTarget.id}` : this.room ? `-room-${this.room.roomid}` : '';
		if (this.broadcasting) {
			if (_optionalChain([room, 'optionalAccess', _3 => _3.battle])) return this.errorReply(`This command cannot be broadcast in battles.`);
	...
	helpticket: {
		...
		submit(target, room, user, connection) {
			...
			let ticket = tickets[user.id];
			const ipTicket = checkIp(user.latestIp);
			if (_optionalChain([ticket, 'optionalAccess', _4 => _4.open]) || ipTicket) {
				if (!ticket && ipTicket) ticket = ipTicket;
				const helpRoom = Rooms.get(`help-${ticket.userid}`);
				if (!helpRoom) {
					// Should never happen
			...
			const introMsg = _utils.Utils.html`<h2 style="margin-top:0">Help Ticket - ${user.name}</h2>` +
		...
		ban(target, room, user) {
			...
			for (const userObj of affected) {
				...
				if (_optionalChain([targetTicket, 'optionalAccess', _5 => _5.open])) targetTicket.open = false;
