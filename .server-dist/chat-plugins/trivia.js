...
let triviaData = {};
try {
	triviaData = JSON.parse(_fs.FS.call(void 0, PATH).readIfExistsSync() || "{}");
...
class Trivia extends Rooms.RoomGame {
	...
	constructor(
		room, mode, category,
		length, questions, isRandomMode = false
	) {
		super(room);
		this.playerTable = {};
		this.gameid = 'trivia' ;
	...
	getWinningMessage(winners) {
		...
		const initialPart = _utils.Utils.html`${p1.name} won the game with a final score of <strong>${p1.player.points}</strong>, and `;
		switch (winners.length) {
		case 1:
			return `${initialPart}their leaderboard score has increased by <strong>${prizes[0]}</strong> points!`;
		case 2:
			return `${initialPart}their leaderboard score has increased by <strong>${prizes[0]}</strong> points! ` +
				_utils.Utils.html`${p2.name} was a runner-up and their leaderboard score has increased by <strong>${prizes[1]}</strong> points!`;
		case 3:
			return initialPart + _utils.Utils.html`${p2.name} and ${p3.name} were runners-up. ` +
	...
	getStaffEndMessage(winners, mapper) {
		...
		this.broadcast(_utils.Utils.html`The game was forcibly ended by ${user.name}.`);
		this.destroy();
	}
}
 ...
 */
const hrtimeToNanoseconds = (hrtime) => {
...
class FirstModeTrivia extends Trivia {
	answerQuestion(answer, user) {
		...
		const buffer = _utils.Utils.html`Correct: ${user.name}<br />` +
...
const commands = {
	new(target, room, user) {
		...
		if (isRandomMode) {
			mode = _utils.Utils.shuffle(['first', 'number', 'timer', 'triumvirate'])[0];
		}
		...
		let questions;
		if (isRandomCategory) {
			const lastCategoryID = toID(_optionalChain([triviaData, 'access', _2 => _2.history, 'optionalAccess', _3 => _3.slice, 'call', _4 => _4(-1), 'access', _5 => _5[0], 'access', _6 => _6.category])).replace("random", "");
	...
	add(target, room, user, connection, cmd) {
		...
		for (const param of params) {
			...
			if (_optionalChain([subs, 'optionalAccess', _7 => _7.some, 'call', _8 => _8(s => s.question === question)]) || _optionalChain([subs, 'optionalAccess', _9 => _9.some, 'call', _10 => _10(q => q.question === question)])) {
	...
	search(target, room, user) {
		...
		const results = (triviaData )[type]
			.filter((q) => q.question.includes(queryString) && !SPECIAL_CATEGORIES[q.category]);
	...
	history(target, room, user) {
		...
		if (!this.runBroadcast()) return false;
		if (!_optionalChain([triviaData, 'access', _11 => _11.history, 'optionalAccess', _12 => _12.length])) return this.sendReplyBox("There is no game history.");

		const games = [...triviaData.history].reverse();
		const buf = [];
		for (const [i, game] of games.entries()) {
			buf.push(_utils.Utils.html`<b>${i + 1}.</b> ${game.mode} mode, ${game.length} length Trivia game in the ${game.category} category.`);
		}
	...
	triviahelp() {
		this.sendReply('|html|<div class="infobox infobox-limited">' + [
			...
			_utils.Utils.html`- <code>/trivia new [mode], [category], [length]</code> - Begin signups for a new trivia game. Requires: + % @ # &`,
			`- <code>/trivia join</code> - Join a trivia game during signups.`,
			_utils.Utils.html`- <code>/trivia start</code> - Begin the game once enough users have signed up. Requires: + % @ # &`,
			`- <code>/ta [answer]</code> - Answer the current question.`,
			_utils.Utils.html`- <code>/trivia kick [username]</code> - Disqualify a participant from the current trivia game. Requires: % @ # &`,
			`- <code>/trivia leave</code> - Makes the player leave the game.`,
			_utils.Utils.html`- <code>/trivia end</code> - End a trivia game. Requires: + % @ #`,
			``,
			`<strong>Question-modifying commands:</strong>`,
			_utils.Utils.html`- <code>/trivia submit [category] | [question] | [answer1], [answer2] ... [answern]</code> - Adds question(s) to the submission database for staff to review. Requires: + % @ # &`,
			_utils.Utils.html`- <code>/trivia review</code> - View the list of submitted questions. Requires: @ # &`,
			_utils.Utils.html`- <code>/trivia accept [index1], [index2], ... [indexn] OR all</code> - Add questions from the submission database to the question database using their index numbers or ranges of them. Requires: @ # &`,
			_utils.Utils.html`- <code>/trivia reject [index1], [index2], ... [indexn] OR all</code> - Remove questions from the submission database using their index numbers or ranges of them. Requires: @ # &`,
			_utils.Utils.html`- <code>/trivia add [category] | [question] | [answer1], [answer2], ... [answern]</code> - Adds question(s) to the question database. Requires: % @ # &`,
			_utils.Utils.html`- <code>/trivia delete [question]</code> - Delete a question from the trivia database. Requires: % @ # &`,
			`- <code>/trivia qs</code> - View the distribution of questions in the question database.`,
			_utils.Utils.html`- <code>/trivia qs [category]</code> - View the questions in the specified category. Requires: % @ # &`,
			_utils.Utils.html`- <code>/trivia clearqs [category]</code> - Clear all questions in the given category. Requires: # &`,
			``,
			`<strong>Informational commands:</strong>`,
			_utils.Utils.html`- <code>/trivia search [type], [query]</code> - Searches for questions based on their type and their query. Valid types: <code>submissions</code>, <code>subs</code>, <code>questions</code>, <code>qs</code>. Requires: + % @ # &`,
