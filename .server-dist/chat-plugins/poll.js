 ...
 class Poll {
	...
	generateVotes(user) {
		...
		if (this.multiPoll) {
			...
			for (const [num, option] of this.options) {
				output += `<div style="margin-top: 5px"><button class="button" style="text-align: left" value="/poll vote ${num}" name="send" title="Vote for ${num}. ${_utils.Utils.escapeHTML(option.name)}">${num}. <strong>${this.getOptionMarkup(option)}</strong></button></div>`;
			}
	...
	getQuestionMarkup() {
		...
		return _utils.Utils.escapeHTML(this.question);
	}

	getOptionMarkup(option) {
		if (this.supportHTML) return option.name;
		return _utils.Utils.escapeHTML(option.name);
	}
