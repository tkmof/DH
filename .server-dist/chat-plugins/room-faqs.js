...
let roomFaqs = {};
try {
	roomFaqs = JSON.parse(_fs.FS.call(void 0, ROOMFAQ_FILE).readIfExistsSync() || "{}");
} catch (e) {
...
function saveRoomFaqs() {
	_fs.FS.call(void 0, ROOMFAQ_FILE).writeUpdate(() => JSON.stringify(roomFaqs));
}
 ...
 const commands = {
	...
	roomfaq(target, room, user, connection, cmd) {
		...
		if (!this.broadcasting && user.can('ban', null, room) && cmd !== 'viewfaq') {
			const src = _utils.Utils.escapeHTML(roomFaqs[room.roomid][topic]).replace(/\n/g, `<br />`);
