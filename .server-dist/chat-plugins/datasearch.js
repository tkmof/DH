 ...
 const commands = {
	...
	dexsearch(target, room, user, connection, cmd, message) {
		...
		return runSearch({
			tar: target,
			cmd: 'dexsearch',
			canAll: !this.broadcastMessage || !!_optionalChain([room, 'optionalAccess', _ => _.settings, 'access', _2 => _2.isPersonal]),
	...
	randommove(target, room, user, connection, cmd, message) {
		...
		return runSearch({
			...
			cmd: 'randmove',
			canAll: !this.broadcastMessage || !!_optionalChain([room, 'optionalAccess', _3 => _3.settings, 'access', _4 => _4.isPersonal]),
			message: (this.broadcastMessage ? "" : message),
		}).then(response => {
	...
	randompokemon(target, room, user, connection, cmd, message) {
		...
		return runSearch({
			tar: targetsBuffer.join(","),
			cmd: 'randpoke',
			canAll: !this.broadcastMessage || !!_optionalChain([room, 'optionalAccess', _5 => _5.settings, 'access', _6 => _6.isPersonal]),
			message: (this.broadcastMessage ? "" : message),
		}).then(response => {
			if (!response.error && !this.runBroadcast(true)) return;
			if (response.error) {
	...
	movesearch(target, room, user, connection, cmd, message) {
		...
		return runSearch({
			tar: target,
			cmd: 'movesearch',
			canAll: !this.broadcastMessage || !!_optionalChain([room, 'optionalAccess', _7 => _7.settings, 'access', _8 => _8.isPersonal]),
			message: (this.broadcastMessage ? "" : message),
		}).then(response => {
			if (!response.error && !this.runBroadcast()) return;
			if (response.error) {
	...
	itemsearch(target, room, user, connection, cmd, message) {
		...
		return runSearch({
			tar: target,
			cmd: 'itemsearch',
			canAll: !this.broadcastMessage || !!_optionalChain([room, 'optionalAccess', _9 => _9.settings, 'access', _10 => _10.isPersonal]),
			message: (this.broadcastMessage ? "" : message),
		}).then(response => {
			if (!response.error && !this.runBroadcast()) return;
			if (response.error) {
	...
	abilitysearch(target, room, user, connection, cmd, message) {
		...
		return runSearch({
			tar: target,
			cmd: 'abilitysearch',
			canAll: !this.broadcastMessage || !!_optionalChain([room, 'optionalAccess', _11 => _11.settings, 'access', _12 => _12.isPersonal]),
			message: (this.broadcastMessage ? "" : message),
		}).then(response => {
			if (!response.error && !this.runBroadcast()) return;
			if (response.error) {
	...
	learn(target, room, user, connection, cmd, message) {
		...
		return runSearch({
			tar: target,
			cmd: 'learn',
			canAll: !this.broadcastMessage || !!_optionalChain([room, 'optionalAccess', _13 => _13.settings, 'access', _14 => _14.isPersonal]),
			message: cmd,
		}).then(response => {
			if (!response.error && !this.runBroadcast()) return;
			if (response.error) {
...
function runMovesearch(target, cmd, canAll, message) {
	...
	const getFullLearnsetOfPokemon = (species) => {
		...
		let usedSpeciesLearnset = Dex.deepClone(Dex.getLearnsetData(usedSpecies.id));
		...
		for (const moveid in dex) {
			...
			for (const recoveryType in alts.recovery) {
				...
				if (recoveryType === "recovery") {
					...
					hasRecovery = (_optionalChain([move, 'access', _15 => _15.zMove, 'optionalAccess', _16 => _16.effect]) === 'heal');
				}
			...
			for (const boost in alts.zboost) {
				...
				if (_optionalChain([zMove, 'optionalAccess', _17 => _17.boost])) {
...
if (!PM.isParentProcess) {
	...
	// @ts-ignore
	require('../../.lib-dist/repl').Repl.start('dexsearch', cmd => eval(cmd)); // eslint-disable-line no-eval, @typescript-eslint/no-var-requires
} else {
