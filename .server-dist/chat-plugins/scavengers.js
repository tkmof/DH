...
class ScavengerHuntDatabase {
 ...
 class ScavengerHunt extends Rooms.RoomGame {
	...
	tryRunQueue(roomid) {
		...
		if (this.room.settings.scavQueue && this.room.settings.scavQueue.length) {
			setTimeout(() => {
				...
				if (!room || room.game || !_optionalChain([room, 'access', _55 => _55.settings, 'access', _56 => _56.scavQueue, 'optionalAccess', _57 => _57.length])) return;

				const next = room.settings.scavQueue.shift();
				const duration = _optionalChain([room, 'access', _58 => _58.settings, 'access', _59 => _59.scavSettings, 'optionalAccess', _60 => _60.defaultScavTimer]) || DEFAULT_TIMER_DURATION;
	...
	validatePlayer(player) {
		...
		if (uniqueConnections > 1 && _optionalChain([this, 'access', _61 => _61.room, 'access', _62 => _62.settings, 'access', _63 => _63.scavSettings, 'optionalAccess', _64 => _64.scavmod, 'optionalAccess', _65 => _65.ipcheck])) {
...
const ScavengerCommands = {
	...
	teamscavs: {
		...
		addplayer(target, room, user) {
			...
			const targetUsers = target.split(',').map(id => Users.getExact(id)).filter(u => _optionalChain([u, 'optionalAccess', _66 => _66.connected])) ;
	...
	modsettings: {
		...
		'update'(target, room, user) {
			...
			const settings = _optionalChain([room, 'access', _67 => _67.settings, 'access', _68 => _68.scavSettings, 'optionalAccess', _69 => _69.scavmod]) || {};
