export const Pokedex: {[k: string]: ModdedSpeciesData} = {
	arbrella: {
		num: 1001,
		name: "Arbrella",
		types: ["Grass", "Ground"],
		baseStats: {hp: 80, atk: 115, def: 100, spa: 80, spd: 80, spe: 75},
		abilities: {0: "Overgrow", H: "Tough Claws"},
		weightkg: 211,
	},
	krachiten: {
		num: 1002,
		name: "Krachiten",
		types: ["Water", "Bug"],
		baseStats: {hp: 90, atk: 120, def: 70, spa: 78, spd: 60, spe: 112},
		abilities: {0: "Torrent", H: "Sniper"},
		weightkg: 59,
	},
	scalaron: {
		num: 1003,
		name: "Scalaron",
		types: ["Fire", "Flying"],
		baseStats: {hp: 90, atk: 70, def: 90, spa: 100, spd: 105, spe: 75},
		abilities: {0: "Blaze", H: "Filter"},
		weightkg: 70,
	},
	rantler: {
		num: 1004,
		name: "Rantler",
		types: ["Normal", "Ice"],
		baseStats: {hp: 68, atk: 118, def: 94, spa: 43, spd: 79, spe: 73},
		abilities: {0: "Intimidate", 1: "Slush Rush", H: "Tough Claws"},
		weightkg: 67,
	},
	woolora: {
		num: 1005,
		name: "Woolora",
		types: ["Fairy"],
		baseStats: {hp: 70, atk: 75, def: 70, spa: 95, spd: 115, spe: 70},
		abilities: {0: "Fluffy", 1: "Rattled", H: "Pastel Veil"},
		weightkg: 50.3,
	},
	albatrygon: {
		num: 1006,
		name: "Albatrygon",
		types: ["Flying"],
		baseStats: {hp: 80, atk: 105, def: 70, spa: 65, spd: 60, spe: 95},
		abilities: {0: "Prankster", 1: "Klutz", H: "Unburden"},
		weightkg: 10.1,
	},
	orchile: {
		num: 1007,
		name: "Orchile",
		types: ["Grass", "Poison"],
		baseStats: {hp: 65, atk: 65, def: 75, spa: 130, spd: 105, spe: 65},
		abilities: {0: "Flower Veil", H: "Metamorphosis"},
		weightkg: 45,
	},
	embuck: {
		num: 1008,
		name: "Embuck",
		types: ["Fire", "Fighting"],
		baseStats: {hp: 90, atk: 125, def: 80, spa: 75, spd: 65, spe: 90},
		abilities: {0: "Intimidate", H: "Flash Fire"},
		weightkg: 113,
	},
	cindoe: {
		num: 1009,
		name: "Cindoe",
		types: ["Fire", "Dark"],
		baseStats: {hp: 80, atk: 75, def: 55, spa: 110, spd: 100, spe: 105},
		abilities: {0: "Natural Cure", H: "Flash Fire"},
		weightkg: 67,
	},
	cobracotta: {
		num: 1010,
		name: "Cobracotta",
		types: ["Grass", "Poison"],
		baseStats: {hp: 80, atk: 90, def: 95, spa: 80, spd: 115, spe: 35},
		abilities: {0: "Mold Breaker", 1: "Weak Armor", H: "Heatproof"},
		weightkg: 68,
	},
	minillow: {
		num: 1011,
		name: "Minillow",
		types: ["Water", "Fairy"],
		baseStats: {hp: 80, atk: 65, def: 75, spa: 95, spd: 95, spe: 80},
		abilities: {0: "Swift Swim", 1: "Dazzling", H: "Adaptability"},
		weightkg: 22,
	},
	crossont: {
		num: 1012,
		name: "Crossont",
		types: ["Bug", "Fighting"],
		baseStats: {hp: 80, atk: 125, def: 80, spa: 60, spd: 100, spe: 80},
		abilities: {0: "Sniper", 1: "Skill Link", H: "Mold Breaker"},
		weightkg: 121,
	},
	torgeist: {
		num: 1013,
		name: "Torgeist",
		types: ["Ghost", "Flying"],
		baseStats: {hp: 55, atk: 65, def: 95, spa: 115, spd: 100, spe: 105},
		abilities: {0: "Cursed Body", 1: "Clear Body", H: "Merciless"},
		weightkg: 9.7,
	},
	platypad: {
		num: 1014,
		name: "Platypad",
		types: ["Grass", "Water"],
		baseStats: {hp: 120, atk: 100, def: 80, spa: 100, spd: 80, spe: 40},
		abilities: {0: "Thick Fat", 1: "Triage", H: "Flower Veil"},
		weightkg: 89,
	},
	lumoth: {
		num: 1015,
		name: "Lumoth",
		types: ["Bug", "Ghost"],
		baseStats: {hp: 60, atk: 55, def: 80, spa: 105, spd: 85, spe: 75},
		abilities: {0: "Levitate"},
		weightkg: 0.5,
	},
	aurorowl: {
		num: 1016,
		name: "Aurorowl",
		types: ["Ice", "Flying"],
		baseStats: {hp: 70, atk: 80, def: 65, spa: 105, spd: 95, spe: 100},
		abilities: {0: "Slush Rush", H: "Technician"},
		weightkg: 15,
	},
	carapex: {
		num: 1017,
		name: "Carapex",
		types: ["Bug", "Flying"],
		baseStats: {hp: 65, atk: 95, def: 155, spa: 65, spd: 90, spe: 55},
		abilities: {0: "Wind Rider", 1: "Mold Breaker", H: "Sturdy"},
		weightkg: 135,
	},
	dojodo: {
		num: 1018,
		name: "Dojodo",
		types: ["Fighting"],
		baseStats: {hp: 90, atk: 115, def: 100, spa: 65, spd: 80, spe: 80},
		abilities: {0: "Iron Fist", 1: "Justified", H: "Stamina"},
		weightkg: 53,
	},
	nunopod: {
		num: 1019,
		name: "Nunopod",
		types: ["Bug", "Ground"],
		baseStats: {hp: 80, atk: 110, def: 125, spa: 70, spd: 75, spe: 65},
		abilities: {0: "Firm Footing", H: "Terrain Power"},
		weightkg: 68,
	},
	zeploom: {
		num: 1020,
		name: "Zeploom",
		types: ["Grass", "Ground"],
		baseStats: {hp: 55, atk: 55, def: 145, spa: 65, spd: 150, spe: 45},
		abilities: {0: "Wind Rider", H: "Levitate"},
		weightkg: 5,
	},
	brawnkey: {
		num: 1021,
		name: "Brawnkey",
		types: ["Steel", "Fighting"],
		baseStats: {hp: 95, atk: 105, def: 105, spa: 95, spd: 90, spe: 50},
		abilities: {0: "Levitate"},
		weightkg: 85,
	},
	salamalix: {
		num: 1022,
		name: "Salamalix",
		types: ["Rock", "Steel"],
		baseStats: {hp: 70, atk: 120, def: 120, spa: 45, spd: 65, spe: 90},
		abilities: {0: "No Guard", 1: "Sand Force", H: "Intimidate"},
		weightkg: 85,
	},
	cinnastar: {
		num: 1023,
		name: "Cinnastar",
		types: ["Rock", "Poison"],
		baseStats: {hp: 110, atk: 95, def: 80, spa: 95, spd: 80, spe: 80},
		abilities: {0: "Mold Breaker", H: "Regenerator"},
		weightkg: 56,
	},
	muabboa: {
		num: 1024,
		name: "MuabBoa",
		types: ["Ground", "Fighting"],
		baseStats: {hp: 65, atk: 110, def: 75, spa: 55, spd: 65, spe: 120},
		abilities: {0: "Anticipation", 1: "Sand Rush", H: "Inner Focus"},
		weightkg: 25,
	},
	volvolpa: {
		num: 1025,
		name: "Volvolpa",
		types: ["Electric", "Ice"],
		baseStats: {hp: 70, atk: 100, def: 65, spa: 65, spd: 92, spe: 133},
		abilities: {0: "Slush Rush", 1: "Volt Absorb", H: "Strong Jaw"},
		weightkg: 27,
	},
	harzodia: {
		num: 1026,
		name: "Harzodia",
		types: ["Psychic"],
		baseStats: {hp: 65, atk: 55, def: 75, spa: 125, spd: 75, spe: 95},
		abilities: {0: "Prankster", 1: "Unburden", H: "Solar Power"},
		weightkg: 35,
	},
	cyllindrake: {
		num: 1027,
		name: "Cyllindrake",
		types: ["Steel", "Dragon"],
		baseStats: {hp: 70, atk: 85, def: 115, spa: 95, spd: 70, spe: 110},
		abilities: {0: "Heavy Metal", 1: "Punk Rock", H: "Wake the Dead"},
		weightkg: 180,
	},
	kodokai: {
		num: 1028,
		name: "Kodokai",
		types: ["Ghost", "Fire"],
		baseStats: {hp: 110, atk: 75, def: 90, spa: 120, spd: 100, spe: 30},
		abilities: {0: "White Smoke", H: "Aroma Veil"},
		weightkg: 50,
	},
	electangle: {
		num: 1029,
		name: "Electangle",
		types: ["Steel", "Electric"],
		baseStats: {hp: 120, atk: 80, def: 110, spa: 80, spd: 90, spe: 30},
		abilities: {0: "Filter"},
		weightkg: 190,
	},
	dolphena: {
		num: 1030,
		name: "Dolphena",
		types: ["Water", "Dragon"],
		baseStats: {hp: 95, atk: 125, def: 100, spa: 60, spd: 79, spe: 81},
		abilities: {0: "Anger Point", H: "mythical Presence"},
		weightkg: 271,
	},
	elemadillo: {
		num: 1035,
		name: "Elemadillo",
		types: ["Steel", "Electric"],
		baseStats: {hp: 67, atk: 94, def: 73, spa: 103, spd: 64, spe: 129},
		abilities: {0: "Weak Armor", 1: "Motor Drive", H: "Stalwart"},
		weightkg: 58,
	},
	axolacred: {
		num: 1036,
		name: "Axolacred",
		types: ["Dragon", "Flying"],
		baseStats: {hp: 60, atk: 75, def: 85, spa: 90, spd: 95, spe: 85},
		abilities: {0: "Wonder Skin", 1: "Shield Dust", H: "Beads of Ruin"},
		weightkg: 33,
	},
	roscenti: {
		num: 1037,
		name: "Roscenti",
		types: ["Bug", "Grass"],
		baseStats: {hp: 80, atk: 115, def: 100, spa: 75, spd: 80, spe: 100},
		abilities: {0: "Flower Veil", 1: "Chlorophyll", H: "Skill Link"},
	},
	blunderbusk: {
		num: 1038,
		name: "Blunderbusk",
		types: ["Water"],
		baseStats: {hp: 68, atk: 67, def: 114, spa: 128, spd: 74, spe: 24},
		abilities: {0: "Mega Launcher", H: "Dry Skin"},
		weightkg: 86,
	},
	barracoth: {
		num: 1039,
		name: "Barracoth",
		types: ["Ice", "Water"],
		baseStats: {hp: 135, atk: 115, def: 105, spa: 55, spd: 65, spe: 35},
		abilities: {0: "Filter", 1: "Thick Fat", H: "Multi-Scale"},
		weightkg: 356,
	},
	jamborai: {
		num: 1040,
		name: "Jamborai",
		types: ["Poison", "Psychic"],
		baseStats: {hp: 65, atk: 55, def: 120, spa: 135, spd: 85, spe: 65},
		abilities: {0: "Clear Body", 1: "Gooey", H: "Water Absorb"},
		weightkg: 84,
	},
	dracoil: {
		num: 1041,
		name: "Dracoil",
		types: ["Dragon", "Flying"],
		baseStats: {hp: 105, atk: 105, def: 85, spa: 95, spd: 60, spe: 80},
		abilities: {0: "mythical Presence", 1: "Gluttony", H: "Marvel Scale"},
		weightkg: 428,
	},
	celespirit: {
		num: 1042,
		name: "Celespirit",
		types: ["Ghost"],
		baseStats: {hp: 75, atk: 61, def: 73, spa: 117, spd: 125, spe: 79},
		abilities: {0: "Levitate", H: "Power Spot"},
		weightkg: 25.8,
	},
	noxtrice: {
		num: 1043,
		name: "Noxtrice",
		types: ["Poison", "Fire"],
		baseStats: {hp: 65, atk: 118, def: 75, spa: 65, spd: 80, spe: 112},
		abilities: {0: "Poison Touch", H: "Flash Fire"},
		weightkg: 73.7,
	},
	faerenheit: {
		num: 1044,
		name: "Faerenheit",
		types: ["Fire", "Fairy"],
		baseStats: {hp: 71, atk: 83, def: 127, spa: 97, spd: 109, spe: 83},
		abilities: {0: "Beast Boost"},
		weightkg: 2.1,
	},
	cellsius: {
		num: 1045,
		name: "Cellsius",
		types: ["Water", "Fairy"],
		baseStats: {hp: 71, atk: 83, def: 83, spa: 109, spd: 127, spe: 97},
		abilities: {0: "Beast Boost"},
		weightkg: 2.4,
	},
	kelven: {
		num: 1046,
		name: "Kelven",
		types: ["Ice", "Fairy"],
		baseStats: {hp: 71, atk: 127, def: 109, spa: 83, spd: 83, spe: 97},
		abilities: {0: "Beast Boost"},
		weightkg: 2.8,
	},
	salaos: {
		num: 1047,
		name: "Salaos",
		types: ["Dark"],
		baseStats: {hp: 75, atk: 85, def: 90, spa: 110, spd: 130, spe: 90},
		abilities: {0: "Heatproof"},
		weightkg: 74,
	},
	morndos: {
		num: 1048,
		name: "Morndos",
		types: ["Dark", "Flying"],
		baseStats: {hp: 110, atk: 90, def: 75, spa: 90, spd: 85, spe: 130},
		abilities: {0: "Synchronize"},
		weightkg: 56,
	},
	pythos: {
		num: 1049,
		name: "Pythos",
		types: ["Dark"],
		baseStats: {hp: 90, atk: 110, def: 130, spa: 85, spd: 90, spe: 75},
		abilities: {0: "Long Reach"},
		weightkg: 178,
	},
	corundell: {
		num: 1050,
		name: "Corundell",
		types: ["Rock", "Electric"],
		baseStats: {hp: 50, atk: 100, def: 150, spa: 100, spd: 100, spe: 100},
		abilities: {0: "Protosynthesis", H: "Crystalline"},
		weightkg: 137,
	},
	quadringo: {
		num: 1051,
		name: "Quadringo",
		types: ["Fairy", "Dragon"],
		baseStats: {hp: 80, atk: 120, def: 95, spa: 100, spd: 130, spe: 75},
		abilities: {0: "Hydration", 1: "Pastel Veil", H: "Wild Roots"},
		weightkg: 156,
	},
	saphor: {
		num: 1052,
		name: "Saphor",
		types: ["Ground", "Normal"],
		baseStats: {hp: 95, atk: 100, def: 105, spa: 100, spd: 105, spe: 95},
		abilities: {0: "Thick Fat", H: "Crystal Heart"},
		weightkg: 638,
	},
	wolf: {
		num: 1053,
		name: "Wolf",
		types: ["Normal", "Dark"],
		baseStats: {hp: 80, atk: 105, def: 100, spa: 105, spd: 100, spe: 110},
		abilities: {0: "Natural Cure", H: "Wild Heart"},
		weightkg: 232,
	},
	soleron: {
		num: 1031,
		name: "Soleron",
		types: ["Electric", "Flying"],
		baseStats: {hp: 70, atk: 75, def: 65, spa: 115, spd: 75, spe: 115},
		abilities: {0: "Awakening"},
		otherFormes: ["Soleron-Awakened"],
		formeOrder: ["Soleron", "Soleron-Awakened"],
		weightkg: 12,
	},
	soleronawakened: {
		num: 1031,
		name: "Soleron-Awakened",
		baseSpecies: "Soleron",
		forme: "Awakened",
		types: ["Electric", "Flying"],
		baseStats: {hp: 70, atk: 105, def: 85, spa: 135, spd: 95, spe: 125},
		abilities: {0: "Awakening"},
		requiredAbility: "Awakening",
		battleOnly: "Soleron",
		weightkg: 24,
	},
	jaegorm: {
		num: 1033,
		name: "Jaegorm",
		types: ["Bug", "Psychic"],
		baseStats: {hp: 60, atk: 65, def: 40, spa: 130, spd: 40, spe: 130},
		abilities: {0: "Schooling"},
		otherFormes: ["Jaegorm-Collective"],
		formeOrder: ["Jaegorm", "Jaegorm-Collective"],
		weightkg: 10,
	},
	jaegormcollective: {
		num: 1033,
		name: "Jaegorm-Collective",
		baseSpecies: "Jaegorm",
		forme: "Collective",
		types: ["Bug", "Psychic"],
		baseStats: {hp: 60, atk: 145, def: 130, spa: 100, spd: 130, spe: 80},
		abilities: {0: "Schooling"},
		requiredAbility: "Schooling",
		battleOnly: "Jaegorm",
		weightkg: 122,
	},
	flocura: {
		num: 1054,
		name: "Flocura",
		types: ["Bug", "Ghost"],
		baseStats: {hp: 55, atk: 95, def: 55, spa: 105, spd: 180, spe: 110},
		abilities: {0: "Xenospore"},
		otherFormes: ["Flocura-Nexus"],
		formeOrder: ["Flocura", "Flocura-Nexus"],
		weightkg: 7.1,
	},
	flocuranexus: {
		num: 1054,
		name: "Flocura-Nexus",
		baseSpecies: "Flocura",
		forme: "Nexus",
		types: ["Bug", "Ghost"],
		baseStats: {hp: 105, atk: 125, def: 105, spa: 135, spd: 180, spe: 50},
		abilities: {0: "Xenospore"},
		requiredAbility: "Xenospore",
		battleOnly: "Flocura",
		weightkg: 999,
	},
};