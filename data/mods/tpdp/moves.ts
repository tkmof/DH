...
export const Moves: {[moveid: string]: MoveData} = {
	...
	dreameater: {
		...
		drain: [1, 2],
		onTryImmunity(target) {
			return target.status['slp'] !== null || target.hasAbility('comatose');
		},
		secondary: null,
		target: "normal",
		type: "Psychic",
	...
	leechseed: {
		...
		condition: {
			...
			onResidual(pokemon) {
				...
				const damage = this.damage(pokemon.baseMaxhp / 8 * (pokemon.hasAbility('overcompensate') ? 4/3 : 1), pokemon, target);
				if (damage) {
					this.heal(damage, target, pokemon);
				}
			},
	...
	lightscreen: {
		...
		condition: {
			duration: 5,
			durationCallback(target, source, effect) {
				if (source?.hasItem(['lightclay', 'fluorite'])) {
					return 8;
				}
				return 5;
			},
	...
	refresh: {
		...
		flags: {snatch: 1},
		onHit(pokemon) {
			if (pokemon.hasStatus(['', 'slp', 'frz'])) return false;
			pokemon.cureStatus();
		},
		secondary: null,
		target: "self",
	...
	rest: {
		...
		onHit(target, source, move) {
			...
			if (!result) return result;
			target.status['slp'].time = 3;
			target.status['slp'].startTime = 3;
	...
	sleeptalk: {
		...
		sleepUsable: true,
		onTry(source) {
			return source.status['slp'] !== null || source.hasAbility('comatose');
		},
		onHit(pokemon) {
			const noSleepTalk = [
				'assist', 'beakblast', 'belch', 'bide', 'celebrate', 'chatter', 'copycat', 'dynamaxcannon', 'focuspunch', 'mefirst', 'metronome', 'mimic', 'mirrormove', 'naturepower', 'shelltrap', 'sketch', 'sleeptalk', 'uproar',
	...
	toxicspikes: {
		...
		condition: {
			...
			onEntryHazard(pokemon) {
				...
				} else if (pokemon.hasType('Steel') || pokemon.hasItem(['heavydutyboots', 'tengugeta'])) {
					return;
				} else if (this.effectState.layers >= 2) {
					pokemon.trySetStatus('tox', pokemon.side.foe.active[0]);
				} else {
